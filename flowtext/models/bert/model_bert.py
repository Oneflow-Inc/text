...
class BertEmbeddings(nn.Module):
    def __init__(self, config):
        ...
        self.register_buffer("position_ids", flow.arange(config.max_position_embeddings).expand((1, -1)))
        self.register_buffer(
            "token_type_ids",
            flow.zeros(self.position_ids.size(), dtype=flow.long, device=self.position_ids.device),
            persistent=False,
        )         
    ...
    def forward(
        ...
        if token_type_ids is None:
            if hasattr(self, "token_type_ids"):
                ...
                token_type_ids = flow.zeros(input_shape, dtype=flow.long, device=self.position_ids.device)
...
class BertSelfAttention(nn.Module):
    ...
    def transpose_for_scores(self, x):
        x = flow.reshape(x, [x.shape[0], x.shape[1], self.num_attention_heads, self.attention_head_size])
    ...
    def forward(
        ...
        context_layer = flow.reshape(context_layer, [context_layer.shape[0], context_layer.shape[1], self.all_head_size])
...
class BertEncoder(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.config = config
        self.layer = nn.ModuleList([BertLayer(config) for _ in range(self.config.num_hidden_layers)])
...
class BertModel(nn.Module):
    ...
    def forward(
        ...
        if attention_mask is None:
            attention_mask = flow.ones((batch_size, seq_length + past_key_values_length), device=device)
