...
class _EncoderBase(Module):
    ...
    def _get_initial_states(self, batch_size: int, num_valid: int, sorting_indices: flow.Tensor) -> Optional[RnnState]:        
        ...
        if batch_size > self._states[0].size(1):
            ...
            for state in self._states:
                zeros = flow.zeros(state.size(0), num_states_to_concat, state.size(2), dtype=state.dtype).to(state.device)
    ...
    def _update_states(self, final_states: RnnStateStorage, restoration_indices: flow.Tensor) -> None:
        new_unsorted_states = [state.index_select(1, restoration_indices) for state in final_states]
        if self._states is None:
            self._states = tuple([state.clone().detach().requires_grad_(True) for state in new_unsorted_states])
        else:
