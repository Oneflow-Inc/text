...
class ElmobiLm(_EncoderBase):
    ...
    def forward(self, inputs, mask):
        ...
        if num_valid < batch_size:
            zeros = flow.zeros(
                ...
                returned_timesteps,
                encoder_dim,
                dtype=stacked_sequence_output.dtype,
            ).to(stacked_sequence_output.device)
    ...
    def _lstm_forward(
        ...
        inputs, batch_lengths = (
            flow.tensor(inputs.numpy()),
            flow.tensor(batch_lengths.numpy()),
        )
        if self.use_cuda:
            inputs = inputs.cuda()
        ...
        for layer_index, state in enumerate(hidden_states):
            ...
            sequence_outputs.append(
                flow.cat([forward_output_sequence, backward_output_sequence], -1)
            )

            final_states.append(
                (
                    flow.cat([forward_state[0], backward_state[0]], -1),
        ...
        final_state_tuple: Tuple[flow.Tensor, flow.Tensor] = (
            flow.cat(final_hidden_states, 0),
...
class LstmbiLm(flow.nn.Module):
    def __init__(self, config, use_cuda=False):
        ...
        self.projection = flow.nn.Linear(
            self.config["encoder"]["dim"],
            self.config["encoder"]["projection_dim"],
            bias=True,
        )
