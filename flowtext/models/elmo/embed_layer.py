    ...
    def __init__(
        ...
        if embs is not None:
            ...
            if n_d != len(embvecs[0]):
                logger.warning(
                    "[WARNING] n_d ({}) != word vector size ({}). Use {} for embeddings.".format(
    ...
    def forward(self, word_inp, chars_inp, shape):
        ...
        if self.word_emb_layer is not None:
            word_emb = self.word_emb_layer(
                flow.Tensor(word_inp).long().cuda()
                if self.use_cuda
                else flow.Tensor(word_inp).long()
            )
        ...
        if self.char_emb_layer is not None:
            ...
            chars_emb = self.char_emb_layer(
                flow.Tensor(chars_inp).long().cuda()
                if self.use_cuda
                else flow.Tensor(chars_inp).long()
            )
    ...
    def __init__(self, config, word_emb_layer, char_emb_layer, use_cuda):
        ...
        if char_emb_layer is not None:
            ...
            self.highways = Highway(
                self.n_filters, self.n_highway, activation=flow.nn.functional.relu
            )
