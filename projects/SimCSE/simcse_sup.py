...
class TrainDataset(Dataset):
    def __init__(self, name, path, tokenizer, max_len = MAXLEN):
...
def loss_sup(y_pred):
    y_true = flow.arange(y_pred.size(0), device=DEVICE)
...
def train(model, train_dataloader, dev_dataloader, optimizer):
    ...
    for step, batch in enumerate(tqdm(train_dataloader), start=1):
        ...
        input_ids = batch['input_ids'].view(bs * 3, -1).to(DEVICE)
        attention_mask = batch['attention_mask'].view(bs * 3, -1).to(DEVICE)
...
if __name__ == '__main__':
    ...
    train_dataloader = DataLoader(TrainDataset('snli', SNIL_TRAIN, tokenizer), batch_size=BATCH_SIZE)
